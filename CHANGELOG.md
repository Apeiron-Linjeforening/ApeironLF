...
- **Konfliktvakt:** har en annen redaktør publisert etter at du åpnet siden, avbrytes angringen med «Noen andre har publisert» og valget «↻ Last inn på nytt», så man aldri ruller tilbake en commit man ikke har sett. Etter angring kommer en toast «✓ Siste publisering angret».
- **Fjernet «Endringer er lokale til du laster ned og pusher»** fra toppen av admin (utdatert siden publisering nå går rett til GitHub).
- Nye filer: `functions/api/github/revert.js` (angrer siste commit ved å peke branchen på forrige tre), `functions/api/github/history.js` (siste commits, driver varselet). Berørte filer: `admin.html`, `admin-github.js`, `VEDLIKEHOLD.md`, `docs/g1-oppsett.md`.

**28.06.26 · Språkvask: fjernet alle tankestreker (—) for bedre setningsflyt**
- **All brukervendt tekst skrevet om uten «—».** Tankestreken ga ofte hakkete setningsflyt og brøt grammatikken. Hver forekomst er erstattet kontekstavhengig: punktum eller komma der det var en setning, kolon der det innledet en liste eller forklaring (f.eks. «PTV: Programtillitsvalgt»), og parentes der det utvidet en forkortelse (f.eks. «S.A.K (Sosiale Arrangement Komité)»). Titler bruker nå «·» som skilletegn.
- **Hele den offentlige siden** dekket: forside, pensum, styret, om oss, begrep, hjelp, merch, marked, galleri, nyheter, oppslagstavla, utmerkelser og oppnåelser, inkludert alle `*-content.js`-filene, «Om oss»-byggeren og søkeindeksen (`search-base.js` / `search-index.js`).
- **Alle 16 admin-paneler** ryddet: instruksjoner, feltetiketter, valg, hjelpetekster og varsler. Søketreff-generatorene (`searchEntries()`) er rettet så ny publisering ikke drar inn tankestreker på nytt.
- **Utviklerdokumentene** `README.md`, `VEDLIKEHOLD.md` og denne `CHANGELOG.md` er språkvasket på samme måte (ankerlenker i innholdsfortegnelsene oppdatert tilsvarende). Rene kodekommentarer er bevisst urørt.
- Berørte filer: samtlige offentlige `*.html` og `*-content.js`, `om.page.js`, `site-search.js`, `search-base.js`, `search-index.js`, `aporetisk-cal.js`, `apeiron-fadder.js`, `apeiron-hero-gallery.js`, `merch-cart.js`, `styles.css`, `admin.html`, `admin-common.js`, samtlige `admin-modules/*.js`, samt `README.md`, `VEDLIKEHOLD.md` og `CHANGELOG.md`.

**28.06.26 · Admin: tryggere publisering (konfliktvarsel + alltid-på lukkevakt), slankere Oversikt + lengre innlogging**
- **Konfliktvarsel før publisering.** To redaktører som redigerte samme fil kunne før stille overskrive hverandre: den som publiserte sist vant, uten varsel. Admin sjekker nå mot GitHub rett før publisering om noen andre har publisert **de samme filene** etter at du åpnet admin, i så fall får du modalen «Noen andre har publisert» med valget **«↻ Last inn på nytt»** (anbefalt) eller **«Publiser likevel»**. Redigerer dere ulike sider, flettes de som før uten varsel. Nytt endepunkt `functions/api/github/latest.js` (siste commit + `compare` for endrede filer).
- **«Sist publisert: «navn · tidspunkt»** vises nå i toppen og i publiser-kortet, så samtidige redaktører kan koordinere seg selv.
- **Lukkevakten kan ikke lenger skrus av for alltid.** «Husk å publisere»-varselet la før et *permanent* flagg i nettleseren når man huket av «Ikke vis igjen», da forsvant også nettleserens egen «du har ulagrede endringer»-vakt for godt. Nå gjelder avhukingen **bare inneværende økt**, og selve lukkevakten (`beforeunload`) fyrer **alltid** når det finnes upubliserte endringer.
- **Upublisert-telleren er nå et alltid-synlig statusmerke** (gull pille med pulserende prikk) i toppen, i stedet for løs grå tekst, så «du har upublisert arbeid» er umulig å overse.
- **Slankere «Slik publiserer du».** Fire tette tekstlinjer er erstattet med **tre ikon-steg** (Logg inn → Rediger → Publiser). Reserveløsningen «↓ Last ned alle endrede» er flyttet bak en sammenleggbar **«Publisering virker ikke?»**, så den ikke konkurrerer visuelt med hovedknappen. Ny linje minner om at man kan **«↺ Angre alle»**. Ordlyden «lagres automatisk» er fjernet (misvisende, siden utkast bevisst er flyktige) → «lever bare i din egen nettleser til du publiserer».
- **Lengre innlogging:** GitHub-innloggingen varte før bare **8 timer**, nå **30 dager**, så styremedlemmer slipper å logge inn på nytt hele tiden (`functions/api/github/callback.js`, `_common.js`).
- Nye filer: `functions/api/github/latest.js`. Berørte filer: `admin.html`, `admin-github.js`, `functions/api/github/callback.js`, `functions/api/github/_common.js`.

**28.06.26 · Admin UX: ryddet oversikt, ⚙ Innstillinger-modal + stylede dialoger**
- **Oppsett-valgene flyttet ut av Oversikt.** «Navigasjon» (Sidemeny ↔ Faner) og «Panelvisning» (Klassisk ↔ Liste + detalj) lå før som to store kort midt på forsiden og blandet seg med selve innholdet. De ligger nå bak en **⚙-knapp øverst til høyre** som åpner en egen «Innstillinger»-modal, med en kort forklaring om at valgene kun er lokale for din nettleser og ikke endrer nettsiden.
- **«Rediger innhold»-kortrutenettet fjernet fra Oversikt.** Den faste venstre-sidemenyen dekker navigasjonen til alle 16 panelene, så kortene var dobbeltarbeid. Panelbeskrivelsene er beholdt som **hover-tekst** på menypunktene, så nye styremedlemmer ikke mister den hjelpen. Oversikten handler nå bare om «Velkommen / Slik publiserer du / Snarveier».
- **«Angre alle endringer» flyttet opp i toppen.** Var en stor boks nederst på Oversikt, nå en kompakt **«↺ Angre alle»**-knapp i headeren ved siden av publiser-kontrollene. Den vises bare når det finnes upubliserte endringer, og er dermed tilgjengelig overalt (også inne i et panel).
- **Native `confirm()`-dialoger byttet ut med stylede modaler.** *Last ned*, *Publiser* og *Tilbakestill alle* (rød/danger) bruker nå samme uttrykk som «husk å publisere»-varselet. **Publiser-modalen viser en «se over endringer»-liste** over de berørte sidene, hver med en **«Se siden ↗»**-lenke for forhåndsvisning før man trykker den grønne knappen.
- **Fikset unødvendig scroll på Oversikt.** Bunnpolstring (70px) + `margin-bottom` på siste boks lagde ~100px tomrom som gjorde forsiden scrollbar uten innhold lenger nede, nå balansert polstring og null bunnmarg på siste element.
- Berørt fil: `admin.html`.

**27.06.26 · Admin UX: venstre-sidemeny som standard, dra-sortering + opprydding i «Liste + detalj»**
- **Gruppert venstre-sidemeny (1A) er nå standard­navigasjon.** De 16 panelene lå før i én lang, overflytende fane-rad (med bla-knapper, fade-kanter og dra-for-å-panorere). De er nå samlet i en fast venstremeny med fire bolker (Forside & nyheter · Om foreningen · Butikk & medlemskap · Sider & oppsett), alltid synlige, man bytter panel direkte uten å innom Oversikt. **Faner beholdt** som valg under Oversikt → «Navigasjon»; valget lagres i nettleseren. Sidemenyen kan **slås sammen til en ikon-stripe** (også husket), og «endret»-prikken vises på menypunktene som på fanene. *(Skallet var faktisk knekt før dette, CSS/grupper/innstilling var skissert, men funksjonene som koblet det sammen manglet, så siden kastet feil ved lasting. Nå komplett.)*
- **Dra-sortering av tilleggsverv (chips)** i både aktivt og arkivert styre: hvert chip-felt har et ⠿-håndtak, dra for å endre rekkefølgen (`renderTags`, delt kode).
- **Aktive styremedlemmer kan flyttes:** medlemmene i venstre-rail-en (Liste + detalj) har ⠿-håndtak og dra-sorteres; rekkefølgen lagres automatisk. Generisk rail-dra lagt i PanelShell (`onReorder` på rail-grupper) så andre paneler kan få det samme.
- **Tilbake til oversikten:** å forlate et panel og komme tilbake åpner nå alltid panelets **oversikt**, ikke det elementet man så på sist (gjelder alle paneler).
- **Grupper lukket som standard** i Liste + detalj, kun grupper man selv har åpnet huskes som åpne.
- **«Tidligere styrer» fikk plass igjen.** Arkiv-kortene fløt vannrett ut av den smale detalj-ruta (rader ~540 px i en ~396 px rute). Felt-radene stables nå loddrett, lange etiketter brytes, og **bildet ligger til venstre med navn/verv/initialer ved siden av** (kompakt, ingen vannrett scroll). Notat-feltet mistet et stort tomrom under (flex-basis ble brukt som høyde), nå tett.
- **Kolonnebredde-velgeren** («Visning · 1/2/3 i bredden») skjules i Liste + detalj på tvers av alle paneler, den ga ikke mening der (ett kort om gangen) og kunne henge igjen som en løs stripe.
- Berørte filer: `admin.html`, `admin-panel-shell.js`, `admin-modules/styret.js`, `admin-modules.css` (cache-versjoner bumpet på CSS/JS-lenkene).

**27.06.26 · Admin: nytt «Liste + detalj»-skall for alle 16 panelene (Legacy beholdt)**
- **Ny redigeringsvisning på tvers av hele Admin-senteret.** Panelene viste tidligere hvert element som et stort kort i full bredde med alle felt åpne samtidig, Styret ble f.eks. ~2 700 px å skrolle gjennom. Nytt felles **PanelShell** gir en smal, søkbar navigator til venstre + ett redigeringsskjema om gangen til høyre. Skjemaene **gjenbruker modulenes eksisterende kort-byggere**, så bilder, chips, angre, lagring og publisering virker akkurat som før.
- **Bryteren ligger i Oversikt → «Panelvisning»:** velg **Liste + detalj** eller **Klassisk (Legacy)**. Valget lagres i nettleseren og deles av alle paneler; et åpent panel tegnes om umiddelbart. **Legacy beholdes** som fullverdig alternativ.
- **Fem rail-typer** dekker at panelene gjør ulike ting: *samlinger* (banner + flere grupper: Styret, Begrep, Footer), *enkeltliste* (Oppnåelser, Utmerkelser, Merch), *status-filter* med segmentert Aktive/Arkiverte/Alle (Nyheter, Oppslagstavla), *seksjoner*, hver sideseksjon blir en rad (Forsiden, Om oss, Medlemskap, Galleri, Marked, Pensum, Hjelp), og *liste-i-detalj* med dra-sortering (Meny, Footer).
- **Skallet ER siden:** full høyde med intern scroll i rail + detalj (ingen død skrolling), «På denne siden»-menyen skjules, og forhåndsvisningen åpnes enten via en **«👁 Forhåndsvisning»**-knapp eller en **fast dokk nederst** (Footer, Meny) så man ser resultatet live mens man redigerer.
- **Sidetekster/banner** er nå en tydelig, gull-markert rad øverst i navigatoren som åpner i detalj-ruten, ikke en innfelt blokk. **«Fold sammen alle / Åpne alle»** lagt til på paneler med flere grupper.
- **Arkivering forbedret** (Oppslagstavla, Nyheter): tydelig **«🗄 Arkivér» / «↩ Hent tilbake»**-knapp i stedet for å klikke en status, og **arkivdato settes automatisk** ved første arkivering og endres aldri ved en utilsiktet «Hent tilbake».
- **Global Ctrl/Cmd+Z** angrer nå både slettinger **og** tillegg (nye seksjoner, blokker, kort, listeelementer). Tekstfelt beholder nettleserens egen angre for skriving.
- Nye filer: `admin-panel-shell.js`. Berørte filer: `admin.html`, `admin-common.js`, `admin-modules.css`, og samtlige `admin-modules/*.js`.
...
- Aporetisk Aften: «Når»-feltet viser «Kommer» ved API-feil og «Dato kommer» ved tom kalender. Genererte torsdagsdatoer vises ikke lenger ved API-feil.
